...
1. **Oyun Döngüsü ve Sahne (`Game.js`)**
   ...
   - `update(time, delta)`: Her frame'de yılanları hareket ettirir, kamerayı zoomlar (skora göre) ve fare tıklama durumunu sunucuya iletir. Her yılan için `snake.postUpdate(delta)` çağrılır (reconciliation/interpolation). Segment + göz güncellemesi ise `_onPostUpdate()` handler'ı aracılığıyla `postupdate` scene event'inde yapılır (aşağıya bak). **Mouse Dead Zone & Blend Zone (Önemli):** `targetAngle` hesaplanmadan önce mouse-head mesafesi ölçülür. `STEER_DEAD_ZONE_PX=35`'ten yakında hiç dönüş yapılmaz (mevcut açı korunur), `STEER_FULL_ZONE_PX=90`'a kadar açı etkisi `0→1` blend ile artar, ötesinde tam yönlendirme. Neden gerekli: Mouse head'e çok yaklaştığında `Angle.Between` sayısal kararsızlığa girer, minicik fare hareketi `targetAngle`'ı yüz derece değiştirebilir ve zigzag / dar kıvrım oluşur. Bu düzeltme hem client prediction'ı hem de sunucuya gönderilen açıyı etkiler. **Mouse Target Angle Clamping & Synchronization (Yeni):** `targetAngle` hesaplanırken fare yönelimi açısı yılanın aktif dönebileceği maksimum fiziksel dönüş hızıyla (`turnSpeed`) derece/kare bazında sınırlandırılır (`Phaser.Math.Clamp`). Bu sayede yılan başının visual rotasyonu ile fiziksel aktif dönebildiği rotasyon mükemmel bir şekilde senkronize olur, fare çok hızlı hareket ettirilse dahi baş visual olarak gövdeden bağımsız/çok daha hızlı dönmez.
