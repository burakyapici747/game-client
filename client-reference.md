...
2. **Yılan Çizimi ve Mantığı (`Snake.js`)**
   ...
   - **SnakeConfig Reconciliation Sabitleri:** `RECONCILIATION_LATERAL_DEADZONE=25px`, `RECONCILIATION_VELOCITY_GAIN=0.8`, `RECONCILIATION_SNAP_DISTANCE=800px`, `RECONCILIATION_MAX_VELOCITY_CAP=0.30`. **Longitudinal düzeltme kaldırıldı** — boost geçişlerinde sunucu farklı hız rejiminden delayed pozisyon gönderir, bu longitudinal velocity offset'ini agresif tetikleyerek path'e anormal büyük/küçük segment sokuyordu → segment sliding. Longitudinal fark client-side prediction tarafından yönetilir. **VELOCITY_GAIN yüksek tutulmamalı (eski=4.0):** Yüksek GAIN, head'i bir frame'de anormal yana kaydırır → `_sampleHeadToPath` bunu path'e yanlış nokta olarak kaydeder → sonraki frame'de sunucuya gönderilen `targetAngle` kayar → sunucu diğer oyunculara bu oyuncuyu yanlış pozisyonda gösterir (2-3x fazla mesafe). Velocity offset ayrıca `calculateBaseSpeed() * MAX_VELOCITY_CAP` ile üst sınıra bağlanmıştır.
